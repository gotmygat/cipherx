...
const VerticalBarsNoise = ({
  ...
  // Use Framer Motion's useInView hook to detect when footer-section (or canvas container) is visible
  const isInView = useInView(viewportRef || containerRef, {
    amount: 0.05, // Trigger when only 5% of the footer-section is visible
    once: false, // Re-trigger when scrolling back
    margin: '0px 0px 0px 0px' // No margin needed since we're watching the entire footer-section
  });
  ...
  const resizeCanvas = useCallback(() => {
    ...
    // Force the parent to use full dimensions first
    const rect = parent.getBoundingClientRect();
    let displayWidth = rect.width;
    let displayHeight = rect.height;

    // If parent is too small, check the grandparent
    if (displayWidth < 500 || displayHeight < 100) {
      const grandParent = parent.parentElement;
      if (grandParent) {
        const grandRect = grandParent.getBoundingClientRect();
        displayWidth = grandRect.width - 48; // Account for padding
        displayHeight = grandRect.height - 40; // Account for padding
      }
    }

    // Ensure minimum size but prefer actual dimensions
    const minWidth = 300;
    const minHeight = 100;
    ...
    // Force the canvas CSS size to match exactly
  ...
  const startAnimation = useCallback(() => {
    if (!isAnimating.current) {
      ...
      console.log('🎬 Canvas animation started - element is in view');
      animate();
    }
  }, [animate]);
  ...
  const stopAnimation = useCallback(() => {
    if (isAnimating.current) {
      ...
      console.log('⏸️ Canvas animation paused - element out of view');
    }
  }, []);
  ...
  useEffect(() => {
    ...
    // Don't auto-start animation here - let the viewport detection handle it
  ...
  }, [resizeCanvas, handleMouseMove, handleMouseDown, handleMouseUp]);
