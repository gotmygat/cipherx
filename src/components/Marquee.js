...
export default function Marquee() {
  ...
  useEffect(() => {
    ...
    const initializeMarquee = () => {
      ...
      if (!marqueeContainer) {
        // Si no encuentra el contenedor, reintenta después de un breve delay
    ...
    // Siempre iniciar las animaciones inmediatamente
    isVisibleRef.current = true;
    
    loops.forEach((tl, index) => {
      // Asegurarse de que no esté pausado y tenga el timeScale correcto
      tl.play();
      tl.timeScale(scrollDirectionRef.current);
    });
    
    // No usar Intersection Observer - mantener animaciones siempre corriendo
    ...
    window.addEventListener("scroll", throttledScroll, { passive: true });
      ...
      // Cleanup
      return () => {
        // observer.disconnect(); // No hay observer que desconectar
    ...
    // Múltiples intentos para asegurar inicialización
    const tryInitialize = (attempt = 1) => {
      
      requestAnimationFrame(() => {
        setTimeout(() => {
          // Verificar que los elementos existan y tengan dimensiones
          const container = document.querySelector('.marquee-container');
          const textSingle = document.querySelector('.text-single');
          const jsTexts = document.querySelectorAll('.js-text');
          
          if (container && textSingle && jsTexts.length > 0) {
            // Verificar que los elementos tengan dimensiones
            const containerRect = container.getBoundingClientRect();
            const firstTextRect = jsTexts[0].getBoundingClientRect();
            
            
            if (containerRect.width > 0 && firstTextRect.width > 0) {
              initializeMarquee();
            } else if (attempt < 5) {
        ...
        }, attempt * 100); // Incrementar delay con cada intento
      });
    };
    ...
    // Detectar cambios de vista desktop/mobile usando resize events
    let resizeTimeout;
    const handleResize = () => {
    ...
    return () => {
      ...
      clearTimeout(resizeTimeout);
      // La función initializeMarquee ya tiene su propio cleanup
    };
  }, []);
